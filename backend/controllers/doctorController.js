...
// this function will remove duplicate slot ad reture the slot by time
...
// this function accepts the object or JSON string
...
export const getDoctors = async (req, res) => {
  try {
    ...
    const total = await Doctor.countDocuments(match); // total number of doctor in DB
...
// to get adeoctor by id is to fetch one doctor
...
export async function getDoctorById(req, res) {
  try {
    // Check for id or doctorId or docId from route params
    const id = req.params.id || req.params.doctorId || req.params.docId;

    if (!id || id === "undefined") {
      return res.status(400).json({
        success: false,
        message: "Invalid or missing Doctor ID in route parameters"
      });
    }
...
export async function updateDoctor(req, res) {
  try {
    ...
    // if exist then update the image else show a warning
    if (req.file?.path) {
      const uploaded = await uploadToCloudinary(req.file.path, "doctors");
      if (uploaded) {
    ...
    delete out.password;
    return res.json({ success: true, data: out }); // updated data reture
  } catch (err) {
