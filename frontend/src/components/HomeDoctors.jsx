...
import { ChevronRight, HandHeart, Medal, MousePointer2Off } from "lucide-react";
...
const HomeDoctors = ({previewCount = 8}) => {
    ...
    //to fetch doctors from the serrver side
    useEffect(() => {
    let mounted = true;
    async function load() {
      setLoading(true);
      ...
      <div className={homeDoctorsStyles.container}>
        <div className={homeDoctorsStyles.header}>
            <h1 className={homeDoctorsStyles.title}>
                Our{" "}
                <span className={homeDoctorsStyles.titleSpan}>Medical Tem</span>
            </h1>
            <p className={homeDoctorsStyles.subtitle}>
                Book appointments quickly with our verified specialization
            </p>
        </div>
        {/* error */}
        {error ? (
                        ...
                        <div className={homeDoctorsStyles.cardBody}>
                            ...
                            <div className={homeDoctorsStyles.buttonContainer}>
                                <div className="w-full">
                                    {doctor.available ? (
                                        <Link to={`/doctors/${doctor.id}`}
                                        style={{doctor: doctor.raw || doctor}}
