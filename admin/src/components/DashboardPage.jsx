...
// Fixed: Removed double slash //api
const PATIENT_COUNT_API = `${API_BASE}/api/appointments/patients/count`;

// Helper function to return a finite number
const safeNumber = (v, fallback = 0) => {
  const n = Number(v);
  return Number.isFinite(n) ? n : fallback;
};

// Details normalization for doctor object
...
const DashboardPage = () => {
  ...
  // Fetch patient count from server
  useEffect(() => {
    let mounted = true;
    async function loadPatientCount() {
      ...
      try {
        ...
        // Comprehensive checks for common API response patterns
        const rawCount =
          body?.count ??
          body?.totalPatients ??
          body?.totalUsers ??
